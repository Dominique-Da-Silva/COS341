...
class SemanticError(Exception):
    ...
    def __init__(self, message, input_file=None, line_number=None, line_content=None):
        # Modified condition to include line number even if input_file is None
...
def type_check_input_file(input_file):
    ...
    param_types = None  # Dictionary to hold possible types for parameters
    ...
    for line_number, line in enumerate(lines, start=1):
        ...
        if line == 'begin' or line == '{':
            ...
            if inside_function and symbol_table.current_scope.scope_type in ['function', 'main']:
                ...
                symbol_table.enter_scope(f"{current_function_name}_body" if current_function_name else "main_body", 
                                        scope_type='function_body', func_name=current_function_name)
            else:
        ...
        if line == 'end' or line == '}':
            if len(current_scope_stack) > 1:
                ...
                if current_scope.scope_type in ['function', 'main']:
                    # Exited function scope, finalize parameter types
                    # Update parameter data types based on inferred types
                    if current_scope.scope_type == 'function':
                        for param_name in current_scope.symbols:
                            if current_scope.symbols[param_name]['type'] == 'var':
                                possible_types = param_types.get(param_name, set()) if param_types else set()
                                if len(possible_types) == 1:
                                    inferred_type = possible_types.pop()
                                else:
                                    inferred_type = 'text'  # Default to 'text' if no info or conflicting types
                                # Update the parameter's data_type in the symbol table
                                current_scope.symbols[param_name]['data_type'] = inferred_type
                inside_function = False
                param_types = None  # Clear param_types
                current_function_name = None
    ...
    if func_call_match:
        ...
        if func_name in built_in_unops:
            ...
            try:
                ...
                # Pass line_number and line_content as keyword arguments
                raise TypeError(str(e), line_number=line_number, line_content=line_content)
